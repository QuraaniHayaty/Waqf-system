...
<body>
    ...
    <script>
        ...
        function loadPropertiesAndLeases() {
            Promise.all([
                fetch('api_properties.php?action=getAll').then(res => res.json()),
        ...
        function renderPropertiesTable(data) {
            ...
            data.forEach((prop) => {
                ...
                globalLeases.forEach(lease => {
                    if(lease.prop && lease.prop.trim() === prop.name.trim()) {
                        totalIncome += parseFloat(lease.amount || 0);
                    }
                });
                ...
                newRow.innerHTML = `
                    ...
                    <td class="prop-income" style="font-weight: bold; color: #27ae60;">${totalIncome.toFixed(2)} ر.ع</td>
                    <td>
            ...
            if(floatsData.length === 0) {
                ...
                floatsData.forEach(f => {
                    ...
                    if(!f.units || f.units.length === 0) {
                        ...
                        f.units.forEach(u => {
                            let matchedLease = globalLeases.find(l => 
                                l.prop && l.prop.trim() === propName.trim() &&
                                l.floor && l.floor.trim() === f.floor.trim() &&
                                l.unit && l.unit.toString().trim() === u.no.toString().trim()
                            );

                            let isRented = matchedLease ? true : false;
                            ...
                            let rentText = isRented ? `${matchedLease.amount} ر.ع` : '-';
                            let filesText = isRented ? `المستأجر: ${matchedLease.tenant} (${matchedLease.amount} ر.ع)` : '<span style="color:#999; font-style:italic;">لا يوجد عقد نشط</span>';
